...
// ========== Helper: glass-pill class (consistent design) ==========
const glassPill = "backdrop-blur-md bg-white/5 border border-white/10 rounded-full px-4 py-2 text-sm hover:bg-white/10 transition-all focus:outline-none focus:ring-2 focus:ring-mora-500/50";

// ========== Search Highlight Component for Pretty View ==========
...
// ========== Tree View with Search Highlighting ==========
const JsonTree = ({ data, searchTerm }: { data: any; searchTerm: string }) => {
  ...
  const renderNode = (node: any, path: string = '$'): React.ReactNode => {
    ...
    if (Array.isArray(node)) {
      ...
      const arrayPreview = `Array [${node.length}]`;
      ...
      return (
        <div>
          <div className="flex items-center gap-1.5 cursor-pointer hover:text-mora-400" onClick={() => toggleExpand(path)}>
            ...
            <span className="text-white/70">{highlightText(arrayPreview)}</span>
          </div>
          {isOpen && (
...
const ApiResponseFormatterPage: React.FC = () => {
  ...
  // ===== Validation & Formatting (with history deduplication) =====
  const validateAndFormat = useCallback((input: string) => {
    ...
    try {
      ...
      const rawStr = input; // keep original raw
      ...
      // History deduplication: avoid saving same input consecutively
      setHistory(prev => {
        const last = prev[0];
  ...
  return (
    <div className="min-h-screen bg-black pt-24 md:pt-32 pb-20 relative overflow-x-hidden">
      {/* Back Button - exact placement & component */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-5 md:px-6">
        ...
        <div className="flex flex-col items-center text-center mb-12 md:mb-16">
          ...
          <h1 className="text-3xl sm:text-4xl md:text-6xl font-semibold tracking-tighter text-white mb-4">
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="bg-[#070707] border border-white/10 rounded-3xl p-4 sm:p-6">
              ...
              <div className="flex flex-wrap gap-2 mb-4 text-xs">
                ...
                {uploadStatus && <div className="bg-green-500/20 text-green-400 px-3 py-1 rounded-full text-xs">{uploadStatus}</div>}
              </div>
            ...
            <div className="bg-[#070707] border border-white/10 rounded-3xl p-4 sm:p-6">
              ...
              {isValid && (
                <div className="mb-4 flex items-center gap-2 bg-black/40 border border-white/10 rounded-full px-4 py-2">
                  ...
                  />
                  {searchTerm && <span className="text-xs text-white/50">{matchCount} matches</span>}
                </div>
              )}
              ...
              {stats ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                  {Object.entries(stats).map(([label, value]) => (
                    <div key={label} className="bg-black/60 rounded-2xl p-4">
                      <div className="text-xs text-white/50 tracking-wider mb-1">{label.replace(/([A-Z])/g, ' $1').trim()}</div>
                      <div className="text-xl md:text-3xl font-semibold tabular-nums">{value}</div>
                    </div>
                  ))}
                </div>
              ) : (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {history.map(item => (
                  <div key={item.id} className="bg-[#070707] border border-white/10 rounded-3xl p-5 hover:border-mora-500/30 transition-all">
                    ...
                    <div className="font-mono text-sm line-clamp-2 text-white/70 mb-4">{item.output.substring(0, 120)}...</div>
        ...
        {history.length === 0 && showHistory && (
          <div className="text-center text-white/40 py-12">No formatting sessions yet</div>
        )}
      </div>
    </div>
  );
