...
);  const MobileNavLink = ({ to, children, onClick, icon: Icon }: React.PropsWithChildren<{ to: string; onClick: () => void; icon: React.ElementType }>) => (    <Link to={to} onClick={onClick} className="flex items-center gap-2 px-3 py-1.5 rounded-xl text-[11px] font-medium text-slate-300 hover:text-mora-400 hover:bg-mora-500/5 border border-transparent hover:border-white/10 transition-all uppercase tracking-wide">    
