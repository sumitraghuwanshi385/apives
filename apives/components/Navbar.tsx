    ...
    <>
      {isLoggingOut && (
        <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex flex-col items-center justify-center">
          ...
          <span className="text-white text-[10px] font-bold uppercase tracking-widest">Logging out...</span>
        </div>
      )}

      <nav className={`fixed z-50 left-1/2 -translate-x-1/2 transition-all duration-700 ${navContainerStyle}`}>
        <div className="mx-auto px-4 md:px-6 w-full">
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center">
              <Link to="/" onClick={() => setIsOpen(false)}>
                <img
                  ...
                  alt="Apives Logo"
                  className="w-9 h-9 md:w-16 md:h-16 object-contain"
                />
              </Link>
            ...
            <div className="hidden lg:flex items-center space-x-3">
              {isAuthenticated ? (
                <div className="flex items-center gap-2">
                  <Link
                    to="/provider"
                    className="flex items-center space-x-2 bg-white/5 hover:bg-mora-500/20 border border-white/10 hover:border-mora-500/50 text-slate-300 hover:text-mora-400 px-5 py-2 rounded-full"
                  >
                    <LayoutDashboard size={14} />
                    <span className="text-[10px] font-bold uppercase tracking-widest">Console</span>
                  </Link>

                  {/* ✅ ADD 2: Sponsor Analytics (DESKTOP) */}
                  {isAdmin && (
                    <Link
                      to="/admin/sponsors"
                      className="flex items-center space-x-2 bg-white/5 hover:bg-mora-500/20 border border-white/10 hover:border-mora-500/50 text-slate-300 hover:text-mora-400 px-5 py-2 rounded-full"
                  ...
                  <button
                    ...
                    className="bg-white/5 hover:bg-red-500/10 hover:text-red-400 text-slate-400 p-2.5 rounded-full border border-white/10"
                  >
                    <LogOut size={14} />
                  </button>
                </div>
                ...
                <Link
                  to="/access"
                  className="flex items-center space-x-2 bg-mora-600 hover:bg-mora-500 text-white px-6 py-2.5 rounded-full"
                >
                  <Terminal size={14} />
            ...
            <button
              ...
              className="lg:hidden w-7 h-7 md:w-10 md:h-10 flex items-center justify-center rounded-full border"
            >
        ...
        {isOpen && (
          <div className="lg:hidden border-t border-white/10 bg-black">
            <div className="px-4 pt-3 pb-4 space-y-1">
              ...
              {isAuthenticated ? (
                <>
                  <MobileNavLink to="/provider" icon={LayoutDashboard} onClick={() => setIsOpen(false)}>
                    Console
                  </MobileNavLink>

                  {/* ✅ ADD 3: Sponsor Analytics (MOBILE) */}
                  {isAdmin && (
                    <MobileNavLink
                      to="/admin/sponsors"
                      icon={ShieldCheck}
                  ...
                  <button
                    onClick={handleLogout}
                    className="w-full flex items-center gap-2 px-3 py-1.5 text-red-400"
                  >
                    <LogOut size={13} /> Logout
                  </button>
                </>
              ) : (
                <MobileNavLink to="/access" icon={Terminal} onClick={() => setIsOpen(false)}>
                  Access Console
                </MobileNavLink>
              )}
            </div>
